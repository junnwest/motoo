...
| ~~2~~ | ✅ **Email delivery — mock 2026-08-11, Resend adapter 2026-08-18.** `EmailProvider` behind the same abstraction as payments/verification. `EMAIL_PROVIDER=resend` + `RESEND_API_KEY` delivers for real; plain `fetch`, no SDK dependency. Throws loudly on a missing key rather than dropping mail silently. Needs a verified sending domain, which does **not** need 사업자등록. | `src/lib/email/resend.ts` |
| ~~3~~ | ✅ **Shipped 2026-08-13, and now enforced (2026-08-18).** Sent on signup, resendable from `/settings`. Donating requires it — the reason is reachability, not identity (본인인증 covers that): a refund decision or cancellation notice sent to an address nobody owns is a promise `/refund` cannot keep. OAuth sign-ins are marked verified on the spot, since Google and Naver only release a confirmed address. **Prerequisite: a real email provider** — the Resend adapter is written, but until `EMAIL_PROVIDER=resend` is set in production the gate cannot be satisfied (OWNER-ACTIONS A5). | `src/lib/mochi.ts` |
...
| ~~10~~ | ✅ **Not required — confirmed 2026-08-18.** This was contingent on analytics landing, and the analytics chosen (Vercel) sets no cookies and involves no third-party host, so no consent banner is needed. It becomes required again the moment a third-party script appears — which is why the Sentry adapter is server-side only. | — |
...
| ~~17~~ | ✅ **Vercel Analytics — 2026-08-18.** Page views and Web Vitals. Chosen over a product-analytics tool *because* of what it does not drag in: no cookies and served from this origin (`/_vercel/insights/*`), so **#10 stays unnecessary** and the enforced CSP needs no widening — `script-src 'self'` already covers it, which is the payoff for having dropped `'strict-dynamic'`. `_vercel` was added to the middleware matcher so beacons aren't 307'd to /onboarding. **Limitation, stated plainly:** it is not a funnel. It answers "is /donate visited and do people leave", not "which step lost them". | `src/app/layout.tsx` |
