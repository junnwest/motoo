...
- [ ] **Known, accepted gap**: the donation-pivot's destructive schema rename
  (`purchasedTotal`/`soldQuantity`/`lifetimeSold` → `mochiEarnedTotal`/`grantedQuantity`/
  `lifetimeGranted`) was only pushed to local dev Postgres, never to Supabase prod —
  Vercel's build doesn't run `db push`, and nobody on this session had
  `.env.production.local`. Owner chose to push code first anyway (DECISIONS 2026-08-10)
  rather than wait. **themotoo.com's mochi-related pages will 500 until the prod schema
  push runs** — see PROGRESS.md's top item and DEPLOYMENT.md → "Schema changes" for the
  exact command.
